...
%$(OUT_EXT): %$(OBJ_EXT) $(COMMON_O) $(LINKER_SCRIPT)
	@# Failed attempt at getting debug symbols.
	@#$(LD) -melf_i386 -o '$(@:$(OUT_EXT)=.elf)' -T '$(LINKER_SCRIPT)' '$<'
	$(LD) --oformat binary -o '$@' -T '$(LINKER_SCRIPT)' '$<' '$(COMMON_O)'

$(COMMON_O): $(COMMON_S)
	$(GAS) $(GAS_FLAGS) -c -o '$@' '$<'

%$(OBJ_EXT): %$(GAS_EXT) $(COMMON_H)
	$(GAS) $(GAS_FLAGS) -c -o '$@' '$<'

%$(OUT_EXT): %$(NASM_EXT)
	nasm -f bin -o '$@' '$<'

# So that directories without a common.X don't have to define it.
$(COMMON_H):
$(COMMON_S):
